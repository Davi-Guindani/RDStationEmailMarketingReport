...
class Graphic:
    ...
    def set_x_bar(self, x_bar):
        self.X_BAR = x_bar

    def get_y_bar(self):
        return self.Y_BAR

    def set_y_bar(self, y_bar):
        self.Y_BAR = y_bar
    ...
    def set_graphics_settings(self, campaign: dict, cliente: Client):
        ...
        p1 = ax.bar(
            self.get_x_bar(),
            self.get_y_bar(),
            color=cliente.get_second_color(),
            width=0.3,
        )
        ...
        ax.set_xticks(
            self.get_x_ticks(),
            color=cliente.get_first_color(),
            labels=self.get_x_ticks(),
        )
        ax.set_yticks(
            self.get_y_ticks(),
            color=cliente.get_first_color(),
            labels=self.get_y_ticks(),
        )
